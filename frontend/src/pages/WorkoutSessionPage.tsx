...
import {
  getNextWorkoutFull, createWorkout, addWorkoutSet, completeWorkout, getExerciseAlternative,
} from '../api/client';
import type {
...
// Упражнения с альтернативой (exercise_id без суффикса _alt)
const EXERCISES_WITH_ALT = new Set([
  'deadlift', 'bench_press', 'seated_cable_row', 'leg_extension',
  'face_pull', 'tricep_pushdown', 'hip_thrust', 'ohp',
  'lat_pulldown', 'leg_press', 'lateral_raise', 'bicep_curl',
]);
...
export default function WorkoutSessionPage() {
  ...
  const handleAlternative = async () => {
    if (!plan || !currentEx || altLoading) return;
    haptic?.selectionChanged();
    ...
    if (isAlt) {
      ...
      // Загрузить альтернативу из БД
      setAltLoading(true);
      try {
        const alt = await getExerciseAlternative(currentEx.exercise_id);
        ...
        setPlan({ ...plan, exercises: newExercises });
        setSetWeight(alt.current_weight || 0);
        setSetReps(alt.reps_max || 10);
      } catch {
        // Альтернатива не найдена — ничего не делаем
        return;
      } finally {
        setAltLoading(false);
      }
      ...
      {stage === 'exercise' && currentEx && (
        <div className="ws-stage">
          <div className="ws-header">
            ...
            {(EXERCISES_WITH_ALT.has(currentEx.exercise_id) || originals[exIdx]) && (
              <button className="ws-alt-btn" onClick={handleAlternative} disabled={altLoading}>
                🔄 {altLoading ? 'Загрузка...' : originals[exIdx] ? 'Вернуть оригинал' : 'Альтернатива'}
              </button>
            )}
          </div>
